...
/*
    TrackingFormerPartyMembers concerns storing characters that were previously in
    the player party within a HashMap for the sake of keeing track of their state.
    Class is crucial for tracking charcaters temporarily removed from the party.
*/
...
public class TrackingFormerPartyMembers 
{
    HashMap<String, GenericCharacter> trackingFormerPartyMembers = new HashMap<>();
    ...
    public void addToCharacterStorage(GenericCharacter character)
    {
        if(character != null)
        {
            trackingFormerPartyMembers.put(character.getName(), character);
        }
    }
    ...
    public void addToCharacterStorage(ArrayList<GenericCharacter> characters)
    {
        for(GenericCharacter element : characters)
        {
            if(element != null)
            {
                trackingFormerPartyMembers.put(element.getName(), element);
            }
        }        
    }
